		...
		while (!timeout) {
			...
			if (mfd->dma->busy == FALSE) {
				...
				if (log_count++ < 100) {
					...
					for (i = 0; i < 24; i += 4) {
						pr_info("%08Xh: %08X %08X %08X %08X\n",
							(unsigned int) msm_mdp_base + 0x40000 + i * 4,
							inpdw(msm_mdp_base + 0x40000 + i * 4 + 0),
							inpdw(msm_mdp_base + 0x40000 + i * 4 + 1),
							inpdw(msm_mdp_base + 0x40000 + i * 4 + 2),
							inpdw(msm_mdp_base + 0x40000 + i * 4 + 3));
					}
				}
				INIT_COMPLETION(mfd->dma->comp);
				timeout = wait_for_completion_timeout(&mfd->dma->comp, HZ/5);
